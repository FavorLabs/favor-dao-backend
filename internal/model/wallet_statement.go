...
func (w *WalletStatement) Get(db *mongo.Database) (*WalletStatement, error) {
...
func (w *WalletStatement) Create(db *mongo.Database) (*WalletStatement, error) {
	err := db.Create(&w).Error

	return w, err
}

func (w *WalletStatement) List(db *mongo.Database, conditions *ConditionsT, offset, limit int) ([]*WalletStatement, error) {
...
func (w *WalletStatement) Count(db *mongo.Database, conditions *ConditionsT) (int64, error) {
	var count int64
	if w.PostID > 0 {
		db = db.Where("post_id = ?", w.PostID)
	}
