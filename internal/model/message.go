...
func (m *Message) Create(db *mongo.Database) (*Message, error) {
	err := db.Create(&m).Error

	return m, err
}

func (m *Message) Update(db *mongo.Database) error {
	return db.Model(&Message{}).Where("id = ? AND is_del = ?", m.Model.ID, 0).Save(m).Error
}

func (m *Message) Get(db *mongo.Database) (*Message, error) {
	var message Message
...
func (c *Message) List(db *mongo.Database, conditions *ConditionsT, offset, limit int) ([]*Message, error) {
...
func (m *Message) Count(db *mongo.Database, conditions *ConditionsT) (int64, error) {
	var count int64
