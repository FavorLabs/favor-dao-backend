...
func (c *CommentContent) MediaContentsByCommentId(db *mongo.Database, commentIds []int64) (contents []string, err error) {
	err = db.Model(c).Where("comment_id IN ? AND type = ?", commentIds, CONTENT_TYPE_IMAGE).Select("content").Find(&contents).Error
	return
}

func (c *CommentContent) DeleteByCommentIds(db *mongo.Database, commentIds []int64) error {
