...
func (c *Comment) Get(db *mongo.Database) (*Comment, error) {
	var comment Comment
...
func (c *Comment) CommentIdsByPostId(db *mongo.Database, postId int64) (ids []int64, err error) {
	err = db.Model(c).Where("post_id = ?", postId).Select("id").Find(&ids).Error
	return
}

func (c *Comment) DeleteByPostId(db *mongo.Database, postId int64) error {
