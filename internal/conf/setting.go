...
func (s *MongoDBSettingS) Dsn() string {
	if s.UserName == "" {
		return fmt.Sprintf("mongodb://%s/%s", s.Host, s.DBName)
	}
	return fmt.Sprintf("mongodb://%s:%s@%s/%s", s.UserName, s.Password, s.Host, s.DBName)
}

func (s *DatabaseSettingS) logLevel() logger.LogLevel {
