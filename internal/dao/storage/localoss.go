...
func (s *localossServant) DeleteObjects(objectKeys []string) (err error) {
	// Loosely handle delete actions, delete all objectKeys if possible, and return only the last error if there is an error
...
func (s *localossServant) SignURL(objectKey string, expiredInSec int64) (string, error) {
	...
	// Fixed: Just make things simple and simple now so return a veiry simple sign url.
